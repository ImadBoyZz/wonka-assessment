 ...
 *   action: a second decision for the same toolCallId returns 409, so a
 *   double-click or replay can never execute twice (idempotency gate).
...
export async function POST(request: Request) {
  ...
  const toolCall = run.toolCalls.find((c) => c.id === body.toolCallId);
  if (!toolCall) {
    return Response.json({ error: `Unknown tool call: ${body.toolCallId}` }, { status: 404 });
  }
  ...
  run.decisions[toolCall.id] = body.decision;

  await appendAudit({
    runId: run.runId,
    actor: "human",
    event: body.decision === "approved" ? "action_approved" : "action_rejected",
